...
public class NotificationRESTController
{
	...
	public ResponseEntity createNotifications(@PathVariable("token") String token,@PathVariable("lat") Float lat,@PathVariable("lon") Float lon,@PathVariable("state") String state) throws ClassNotFoundException, SQLException
	{
		...
		{ // si l'id_user existe
			...
			{ 
				...
				//s'il y a deja une notif en cours, on update les positions, la date et le flag_processing =0
				{ 
			...
			else // si la sortie de zone est terminee on update les positions, la date et le flag_processing =1
			{ 
	...
	// pour appeler l'api 
	    ...
	    public ResponseEntity<ListNotification> getAllNotificationsByUser(@PathVariable("token") String token) throws ClassNotFoundException, SQLException
	    {
	    	...
	    	//creation area en local
	    	 ...
	    	 if(id_user!=null)
	    	 { // a partir de l'id_user on retrouve la liste des zones associees a cet id
