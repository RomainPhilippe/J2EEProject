			...
			.ajax({
				...
				success : function(data) {
					...
					for (var i = 0; i < length; i++) {
						...
						// data['area'][i]['latitude']);
						// console.log("longitude : "+
						// data['area'][i]['longitude']);
						var myLatlng = new google.maps.LatLng(latitude,
								longitude);
						listMarker[i] = createMarker(myLatlng, "m" + i, map,
								nameArea, labelArea, id_area, latitude,
								longitude, distance);
...
function createMarker(pos, t, map, title, description, id, latitude, longitude,
		distance) {
	...
	var contentString = '<a href="detailObject.action?id=' + id
			...
			+ '<div id="bodyContent">' + '<p>' + description + '</p>'
			+ '</div>' + '</div>';
